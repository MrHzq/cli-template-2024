...
// 统一的运行流程方法
const runStep = (list, _failType = "fail", config = {}) => {
  let everySuccess = false;
  for (let index = 0; index < list.length; index++) {
    const { fun, desc, failType } = list[index];

    const finalType = failType || _failType;

    const currDesc = typeof desc === "function" ? desc() : desc;

    const stepSpinner = new Spinner(index + 1 + "、" + currDesc);

    const res = typeof fun === "function" ? fun() : "";
